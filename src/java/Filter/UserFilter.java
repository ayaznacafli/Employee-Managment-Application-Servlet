			...
			if(user==null){
				...
				if(action!=null && action.contains("/")){
				    String ArrayAction[]=action.split("/");
				         if(ArrayAction!=null && ArrayAction.length>0){
					  action=ArrayAction[ArrayAction.length-1];
				           }
				}
				UserSql userSql = new UserSql();
				int actionId=userSql.getActionId(action);
				
				if(actionId!=-1){
				boolean isAccessAction = userSql.isAccessForAction(roleId, actionId);
					if(isAccessAction){
